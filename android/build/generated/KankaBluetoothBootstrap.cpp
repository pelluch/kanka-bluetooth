...
#define TAG "com.ewin.kanka.bluetooth"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> KankaBluetooth_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("KankaBluetooth.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::KankaBluetoothBindings::lookupGeneratedInit(
...
static void KankaBluetooth_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(KankaBluetooth_getBinding)->GetFunction());
}

static void KankaBluetooth_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::KankaBluetoothBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry KankaBluetoothBinding = {
	"com.ewin.kanka.bluetooth",
	KankaBluetooth_init,
	KankaBluetooth_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_com_ewin_kanka_bluetooth_KankaBluetoothBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.ewin.kanka.bluetooth", &KankaBluetoothBinding);
	titanium::KrollBindings::addExternalLookup(&(::KankaBluetoothBindings::lookupGeneratedInit));
}
